async () => {
  ...
  return JSON.stringify(g.dataSource.view().filter(x => dt(x) >= '2026-09-12').map(x => ({n:x.Number, d:dt(x), loc:x.Location, id:x.TransactionId, st:x.ApprovalStatus, amt:x.Amount})));
}
