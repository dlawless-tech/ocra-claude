...
const it = (await o.querySearch('104-06')).find(x=>/Grub/i.test(x.display));
...
return [el.value, Array.from(sec.querySelectorAll('button')).map(x=>{const v=w.angular.element(x).scope().valuePair; return v.display+':'+v.wanted;})];
}
