...
# session is left alone. Credentials come from ~/.claude/norms-credentials.md,
# which lives outside any repo and is never copied into one.
#
# Usage: r365-login.sh <session>
set -u
S="$1"
CREDS="$HOME/.claude/norms-credentials.md"
[ -f "$CREDS" ] || { echo "FAIL: no $CREDS"; exit 1; }

U=$(playwright-cli -s=$S eval "() => location.href" 2>&1 | sed -n '/### Result/,/### Ran Playwright/p' | sed '1d;$d' | tr -d '\n"')
# host only: the login URL carries an encoded norms.restaurant365.com in its ReturnUrl
H=${U#*://}; H=${H%%/*}
case "$H" in norms.restaurant365.com) echo "already authenticated: $U"; exit 0;; esac
...
playwright-cli -s=$S open --headed https://norms.restaurant365.com/react/accounting >/dev/null 2>&1
sleep 8
