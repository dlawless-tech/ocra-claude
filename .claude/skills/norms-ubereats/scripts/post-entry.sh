...
for attempt in 1 2 3; do
  ...
  LN=$(grep -n 'gridcell "a/r ubereats - payout"' $TMP | head -1 | cut -d: -f1)
  if [ -n "${LN:-}" ]; then
...
fill "a/r ubereats - payout"  credit "$(J arCredit)"
fill "marketing"              debit  "$(J marketing)"
fill "uber fees"              debit  "$(J fees)"
fill "net chargeback amount"  debit  "$(J ncDr)"
fill "net chargeback amount"  credit "$(J ncCr)"
fill "difference"             debit  "$(J diffDr)"
fill "difference"             credit "$(J diffCr)"

WANT=$(J totDr)
TOT=$(playwright-cli -s=$S eval "() => { const L=['a/r ubereats - payout','marketing','uber fees','net chargeback amount','difference']; const rows=Array.from(document.querySelectorAll('tr')).map(r=>Array.from(r.cells||[]).map(c=>c.innerText.trim())); let dr=0,cr=0; for(const l of L){ const t=rows.find(x=>x.includes(l)); if(!t) return 'MISSING:'+l; dr+=parseFloat((t[3]||'0').replace(/,/g,''))||0; cr+=parseFloat((t[4]||'0').replace(/,/g,''))||0; } return dr.toFixed(2)+'/'+cr.toFixed(2); }" 2>&1 | res | tr -d '"')
...
sleep 14
VER=$(playwright-cli -s=$S eval "() => { const rows=Array.from(document.querySelectorAll('tr')).map(r=>Array.from(r.cells||[]).map(c=>c.innerText.trim())); const g=l=>{const r=rows.find(t=>t.includes(l)); return r?r[3]+'/'+r[4]:'?';}; return g('a/r ubereats - payout')+' mk '+g('marketing')+' fe '+g('uber fees')+' nc '+g('net chargeback amount')+' df '+g('difference'); }" 2>&1 | res | tr -d '"')
