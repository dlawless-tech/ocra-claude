#!/bin/bash
# Fill, save, verify and approve one Bowery DoorDash journal entry.
#
# Usage: ENTRY_DATE=9/6/2026 post-entry.sh <session> <r365-location> [work-json]
#   work-json defaults to $BOWERY_DOORDASH_WORK, else ./work.json
#   ENTRY_DATE is the entry date as R365 renders it, and is required. Bowery's
#   DoorDash period runs Mon-Sun, so this is the Sunday that ends the period.
...
WORK="${3:-${BOWERY_DOORDASH_WORK:-./work.json}}"
